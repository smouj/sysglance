...
check('palette commands are navigation-only or explicit UI actions', renderer.includes('PALETTE_COMMANDS') && renderer.includes('Show system status') && renderer.includes("action: 'taskManager'") && renderer.includes("action: 'sleep'") && main.includes("ipcMain.handle('control:open'"));
check('control actions are allow-listed, argument-safe and confirmed when destructive', main.includes("ms-settings:network") && main.includes("execFile('taskmgr.exe', [], { windowsHide: true }") && main.includes("['user32.dll,LockWorkStation']") && main.includes("shutdown.exe") && main.includes('CONTROL_CONFIRMATIONS') && !main.includes('exec('));
