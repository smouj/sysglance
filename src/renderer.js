...
(function () {
  ...
  // Writing textContent unconditionally forces a style/layout pass even when the
  // value is identical. Every hot value goes through these two helpers instead.
  ...
  // Minute precision is enough for a clock; a 1 s timer is a repaint per second
  // for nothing. We also refresh on focus/visibility change.
  ...
  var coreFills = null; // cached per-core bars, rebuilt only when the core count changes
  ...
  function applyUpdate() {
    ...
    // Nothing to paint into: drop the whole update while the window is hidden.
    ...
    if (data.cpu) {
      ...
      if (data.cpu.perCore && data.cpu.perCore.length) {
        var cores = data.cpu.perCore;
        // Build the markup once; rebuilding innerHTML every cycle reparses the grid
        // and repaints every bar.
        if (coreFills === null || coreFills.length !== cores.length) {
          var html = '';
          for (var i = 0; i < cores.length; i++) {
            html += '<div class="core-bar" title="Core ' + i + '"><div class="core-fill"></div></div>';
          }
          dom.cpuPercore.innerHTML = html;
          coreFills = dom.cpuPercore.querySelectorAll('.core-fill');
        }
        for (var j = 0; j < cores.length; j++) {
          var v = Math.min(cores[j], 100);
          var fill = coreFills[j];
          var cls = 'core-fill ' + loadClass(v);
          if (fill.className !== cls) fill.className = cls;
          if (fill.style.width !== v + '%') fill.style.width = v + '%';
        }
      }
    }
