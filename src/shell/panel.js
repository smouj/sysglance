...
(function () {
  ...
  try { ipcRenderer = require('electron').ipcRenderer; } catch (e) { /* not electron */ }
  ...
  var HTML =
    '<div class="section" id="sec-shell">' +
      ...
      '<div class="shell-group">' +
        '<div class="shell-label">Taskbar blur <span id="shell-blur-status" class="info-small">—</span></div>' +
        '<div class="shell-actions">' +
          '<button class="shell-btn" id="shell-blur-apply">Apply</button>' +
          '<button class="shell-btn" id="shell-blur-start">Resident</button>' +
          '<button class="shell-btn" id="shell-blur-stop">Stop</button>' +
        '</div>' +
      '</div>' +
  ...
  var el = {
    ...
    blurStatus: document.getElementById('shell-blur-status'),
    blurApply: document.getElementById('shell-blur-apply'),
    blurStart: document.getElementById('shell-blur-start'),
    blurStop: document.getElementById('shell-blur-stop'),
  ...
  function say(text, kind) {
    ...
    // make it explode when the user tries to act without an explorer restart.
  ...
  function render(st) {
    ...
    var blur = st.blur || {};
    el.blurStatus.textContent = blur.running ? 'resident · running'
      : (blur.helperExists ? 'idle' : 'helper not built');
    el.blurStop.disabled = !blur.running;
    el.blurApply.disabled = !blur.helperExists;
    el.blurStart.disabled = !blur.helperExists;
  ...
  async function call(channel, arg) {
    try {
      var payload = await ipcRenderer.invoke(channel, arg);
  ...
  async function refresh() {
    try {
      render(await ipcRenderer.invoke('shell:taskbar:getState'));
    } catch (err) {
  ...
  el.position.addEventListener('click', async function (ev) {
    ...
    if (!btn) return;
    say('Applying…');
    var r = await call('shell:taskbar:setPosition', btn.dataset.pos);
    if (!r) return;
  ...
  el.autohide.addEventListener('change', async function () {
    ...
    say('Applying…');
    var r = await call('shell:taskbar:setAutoHide', on);
    if (!r) return;
    if (r.ok === false) return say('✖ ' + r.error, 'err');
    say(r.changed
  ...
  el.dark.addEventListener('change', async function () {
    ...
    say('Applying…');
    var r = await call('shell:theme:setDark', on);
    if (!r) return;
    if (r.ok === false) return say('✖ ' + r.error, 'err');
  ...
  el.accentFromWallpaper.addEventListener('click', async function () {
    ...
    var r = await call('shell:accent:fromWallpaper');
    if (!r) return;
    if (r.ok === false) return say('✖ ' + r.error, 'err');
  ...
  el.accentAuto.addEventListener('click', async function () {
    ...
    var r = await call('shell:accent:auto');
    if (!r) return;
    if (r.ok === false) return say('✖ ' + r.error, 'err');
  ...
  el.wallpaperBrowse.addEventListener('click', async function () {
    var r = await call('shell:wallpaper:pick');
  ...
  el.wallpaperApply.addEventListener('click', async function () {
    ...
    var r = await call('shell:wallpaper:apply', p);
    if (!r) return;
    if (r.ok === false) return say('✖ ' + r.error, 'err');
    say('Wallpaper applied (registry + SystemParametersInfo).', 'ok');
  });

  el.blurApply.addEventListener('click', async function () {
    say('Applying taskbar blur…');
    var r = await call('shell:blur:apply', {});
    if (!r) return;
    say(r.ok ? 'Blur applied: ' + (r.output || 'ok') : '✖ ' + r.error, r.ok ? 'ok' : 'err');
  });

  el.blurStart.addEventListener('click', async function () {
    say('Starting resident watcher…');
    var r = await call('shell:blur:start', {});
    if (!r) return;
    say(r.ok ? 'Resident watcher running (pid ' + r.pid + '), re-applies every 2s.' : '✖ ' + r.error, r.ok ? 'ok' : 'err');
  });

  el.blurStop.addEventListener('click', async function () {
    var r = await call('shell:blur:stop');
    if (!r) return;
    say(r.ok ? 'Resident watcher stopped.' : '✖ ' + r.error, r.ok ? 'ok' : 'err');
  });
  ...
  el.message.addEventListener('click', async function (ev) {
    ...
    var r = await call('shell:taskbar:restartExplorer');
    if (!r) return;
  ...
  ipcRenderer.on('shell-config-changed', function () { refresh(); });
