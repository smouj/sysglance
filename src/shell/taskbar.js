...
// Replaces TranslucentTB (taskbar look) and Rainmeter (desktop
// tweaks) using only APIs that ship with Windows. No npm deps.
//
...
const HELPER = path.join(__dirname, '..', 'native', 'trayblur', 'SysGlanceTrayBlur.exe');
...
function helperPath() {
  if (process.env.SYSGLANCE_TRAYBLUR_HELPER) return process.env.SYSGLANCE_TRAYBLUR_HELPER;
  return HELPER;
}
...
async function systemParametersInfoWallpaper(filePath) {
  ...
  if (!fs.existsSync(helper)) {
    return { ok: false, error: 'native helper not built — run scripts/build-native.ps1 (expected at ' + helper + ')' };
  }
  return new Promise((resolve) => {
    execFile(helper, ['--wallpaper=' + filePath], { windowsHide: true, timeout: 15000 }, (err, stdout, stderr) => {
      if (err) return resolve({ ok: false, error: (stderr || err.message || '').trim(), output: (stdout || '').trim() });
...
async function refreshThemeChange() {
  const helper = helperPath();
  if (!fs.existsSync(helper)) return { ok: false, error: 'native helper not built — run scripts/build-native.ps1' };
  return new Promise((resolve) => {
    execFile(helper, ['--refresh-theme'], { windowsHide: true, timeout: 10000 }, (err, stdout, stderr) => {
